...
**Dernière mise à jour :** 12 juin 2026 (**checklist critères mot de passe** inscription/reset ; précédent : connexion Google OAuth…)
...
- **Taille :** ~450 lignes
- **Dernière mise à jour :** 12 juin 2026 (§4.1 checklist mot de passe ; précédent : §4.1 Google OAuth…)
...
- **Taille :** ~2010 lignes
- **Dernière mise à jour :** 12 juin 2026 (**PasswordRequirements**, **NewPasswordField** ; précédent : SocialAuthButtons OAuth…)
...
- **Taille :** ~195 lignes
- **Dernière mise à jour :** 12 juin 2026 (**`auth.passwordRequirements`** checklist mot de passe ; précédent : checkoutPrerequisites…)
...
- **Contenu :** Maquettes HTML et README d’index pour l’**abonnement plateforme coach** (page Mon Abonnement, Mes athlètes bloqué, modale Stripe, **carte abo actif / essai**, **défaut de paiement sans grâce**, **adresse de facturation**, **prérequis Checkout** **`MOCKUP_US_MYSA_CHECKOUT_PREREQ_01…03`**) ; **README** inclut pointeurs d’implémentation (`coach-platform-subscription`, **`coachPlatformCheckoutPrerequisitesActions`**, **`coachPlatformBillingAddressActions`**, **`lib/coachPlatformCheckoutPrerequisites.ts`**, **`persistCoachPlatformStripeBillingForCoach`**, migrations **074**–**076**). Maquette **grâce 3 j** → **`docs/archive/design-coach-platform-subscription-grace/`** (archivée).
- **Utiliser pour :** Contexte UI historique et référence livrée ; le détail produit / technique à jour est dans **`Project_context.md`** §4.4 et **`docs/DESIGN_SYSTEM.md`**.
- **Dernière mise à jour :** 9 juin 2026 (US-MYSA-CHECKOUT-PREQ-01…03 **livré** ; précédent : Phase 2 Designer…)
...
**Refonte modale workout – Objectif/Réalisé hero + feedback coloré (archivés 8 mai 2026) :**
- `docs/archive/design-workout-modal-revamp/` — MOCKUP_US1_WORKOUT_TARGET_ACTUAL_CARDS.html, MOCKUP_US2_WORKOUT_FEEDBACK_SUMMARY.html, MOCKUP_US3_COACH_READONLY_WORKOUT_MODAL.html, MOCKUP_US4_ATHLETE_WORKOUT_MODAL.html.
- **Raison :** Feature livrée ; modale workout (athlète + coach lecture seule) : cartes « Objectif vs Réalisé » côte-à-côte (`WorkoutTargetActualCards`) avec hero dark, chips delta `±M:SS/km` (allure mm:ss), valeurs réalisées en blanc, commentaire athlète intégré en bas de la card Réalisé avec séparateur, alignement vertical des rangées (`min-h-7`), badge sport entête réduit (~15 %) ; tuiles feedback (`WorkoutFeedbackSummary`) distribuées full-width avec couleurs sémantiques par valeur (Ressenti / Plaisir 1–5 rouge → vert, Intensité 1–10 vert → orange → rouge) ; même dégradé appliqué au picker athlète (`WorkoutFeedbackSection`) ; couleurs centralisées dans `lib/workoutFeedbackColors.ts`, formats centralisés dans `lib/workoutFormatting.ts` (tests Vitest). Comportement décrit dans **Project_context.md §4.5** (Athlete can / Coach can / Feedback colors), **docs/DESIGN_SYSTEM.md** (WorkoutTargetActualCards, WorkoutFeedbackSummary, WorkoutFeedbackSection, Feedback colors), **docs/I18N.md** (`workouts.summary.*`).
...
**Objectif de temps facultatif + édition objectif (archivés 16 mars 2026) :**
...
- **Doc :** `Project_context.md` §4.4, `docs/design-coach-platform-subscription/README.md`, `docs/DESIGN_SYSTEM.md` (v1.44), `docs/I18N.md`, `DOCS_INDEX.md`.
- **Archivage :** aucun (maquettes **US-MYSA-CHECKOUT-PREQ-*** conservées comme référence livrée).
- **Pages légales / confidentialité :** aucune modification (même finalités identité / facturation ; pas de nouvelle finalité).

✅ **9 juin 2026 – Abonnement plateforme coach (prérequis Checkout) – Mode Analyste (design Phase 2, pas de livraison code) :**
- **Design validé PO :** **US-MYSA-CHECKOUT-PREQ-01…03** — avant redirect Stripe, prénom + nom + adresse facturation Canada obligatoires ; **modale offres** = bascule contenu (`CoachPlatformSubscribeOffersModal`) ; **page Mon Abonnement** = modale dédiée (`md`) ; formulaire commun (tous champs visibles, pré-remplissage) ; maquettes **`MOCKUP_US_MYSA_CHECKOUT_PREREQ_01_MODAL_SWAP.html`**, **`02_SUB_PAGE_MODAL.html`**, **`03_FORM.html`**.
- **Doc :** `docs/design-coach-platform-subscription/README.md`, `Project_context.md` §4.4 (mention *à implémenter*), `docs/DESIGN_SYSTEM.md` (v1.43 — spec composants), `docs/I18N.md` (namespace prévu **`checkoutPrerequisites`**), `DOCS_INDEX.md`.
- **Archivage :** aucun.
...
- **Doc :** `Project_context.md` §4.4, `DEPLOYMENT_NOTES.md`, `docs/DESIGN_SYSTEM.md` (v1.42), `docs/I18N.md`, `docs/design-coach-platform-subscription/README.md`, `DOCS_INDEX.md`.
- **Archivage :** aucun (maquettes design **US-MYSA-CANCEL-***, **US-MYSA-SUB-UI-02-*** conservées comme référence livrée).
- **Pages légales / confidentialité :** aucune modification proposée (gestion abonnement = prolongement facturation Stripe existante ; pas de nouvelle finalité de traitement sans validation PO).

✅ **20 mai 2026 – Abonnement plateforme coach (essai non renouvelable) – Mode Analyste :**
- **Livraison (réf. code) :** migration **`075_coach_platform_trial_consumptions`** ; **`lib/coachPlatformTrialEligibility.ts`** (`trialEligible`, **`syncCoachPlatformTrialConsumptionFromStripeSubscription`**) ; **`COACH_PLATFORM_SUBSCRIPTION_TRIAL_CAMPAIGN_ID`** (+ fallback **`platform-default`**) ; **`CoachPlatformOfferGrid`** masquage silencieux si essai consommé ; garde Checkout ; webhook + repli **`verifyCoachPlatformCheckoutSession`** ; tests **`lib/coachPlatformTrialEligibility.test.ts`**.
- **Doc :** `Project_context.md` §4.4, `README.md`, `DEPLOYMENT_NOTES.md`, `docs/DESIGN_SYSTEM.md` (v1.41), `docs/design-coach-platform-subscription/README.md`, `DOCS_INDEX.md`.
- **Archivage :** aucun.
- **Pages légales / confidentialité :** aucune modification proposée (règle métier commerciale ; pas de nouvelle finalité de traitement).

✅ **20 mai 2026 – Abonnement plateforme coach (essai, tuiles, page) – Mode Analyste (conversation complète) :**
- **Livraison (réf. code) — essai catalogue (scénario A) :** variable **`COACH_PLATFORM_SUBSCRIPTION_TRIAL_DAYS`** → **`lib/coachPlatformSubscriptionTrial.ts`** (`getCoachPlatformSubscriptionTrialDays`, **`computeCoachPlatformTrialRemainingDays`**) ; **`loadCoachPlatformCatalogForEnv`** / **`loadCoachPlatformCatalogForCoach`** exposent **`subscriptionTrialDays`** ; **`CoachPlatformOfferGrid`** : badge **`trialCatalogBadgeFreeDays`**, ligne **`trialCatalogTrialLine`** ; **`createCoachPlatformCheckoutSession`** : **`subscription_data.trial_period_days`** si > 0.
- **Livraison — tuiles offre :** **`lib/coachMsaOfferDisplay.ts`** (`tagline`, **`features`** depuis **`coachMsaOffers.byPriceId`**) ; cartes `rounded-2xl`, prix **`text-3xl`**, puces check ; tests **`lib/coachMsaOfferDisplay.test.ts`**.
- **Livraison — page Mon abonnement :** carte statut **uniquement** `active` / `trialing` (jours restants, **puis** tarif, prochain prélèvement) ; **plus** de bandeau gris « sans abo » / copie défaut de paiement sur la page ; grille offres si **pas** `active`/`trialing` ; **sans** titre ni intro visibles (**`offersTitle`** **`sr-only`** ; clés **`offersSectionHeading`** / **`offersSectionIntro`** supprimées). Fichiers : **`page.tsx`**, **`CoachPlatformOfferGrid.tsx`**, **`CoachPlatformSubscriptionOffers.tsx`**, **`CoachPlatformSubscribeOffersModal.tsx`**, **`coachPlatformActions.ts`**, **`stripeCoachPlatformCatalog.ts`**, **`messages/fr.json`** & **`en.json`**.
- **Doc :** `Project_context.md` §4.4, `README.md`, `DEPLOYMENT_NOTES.md`, `docs/DESIGN_SYSTEM.md` (v1.40), `docs/I18N.md`, `docs/design-coach-platform-subscription/README.md`, `DOCS_INDEX.md`.
- **Archivage :** aucun (maquettes **US-COACH-MYSA-01**, **US-MYSA-TRIAL-01**, **US-MYSA-SUB-ACCESS-01** : en-tête « Offres disponibles » possible — écart noté dans README design).
- **Pages légales / confidentialité :** aucune modification (pas de nouvelle finalité ; essai = condition commerciale Stripe existante).
- **Nettoyage i18n (suite) :** clés orphelines **`coachMsaSubscription`** retirées des JSON (bandeaux sans abo, défaut de paiement, essai `trialFree`, statuts Stripe non affichés) ; **`billingAddress.cardTitle`** / **`formTitle`** retirées.
...
- **Doc :** `Project_context.md` §4.0 (tableau + paragraphes « Marketing home », en-tête public), `docs/DESIGN_SYSTEM.md` § **PublicOrDashboardHeader** + **Fichiers clés** + version, `DOCS_INDEX.md`.
- **Archivage :** aucun.
...
- **Fichiers :** `supabase/migrations/056_goals_target_time.sql`, `types/database.ts`, `lib/goalResultUtils.ts`, `app/[locale]/dashboard/objectifs/actions.ts` (addGoal, updateGoal), `GoalEditModal.tsx`, `ObjectifsTable.tsx`, `CalendarView.tsx`, `CoachAthleteCalendarPage.tsx`, `messages/fr.json`, `messages/en.json`.
- **Doc :** Project_context.md §4.7 (Goals : objectif de temps, édition, affichage), §5 (goals target_time_*), DESIGN_SYSTEM (§ TileCard / Page Objectifs, GoalEditModal), I18N (goals.targetTime*, editGoal, achieved), project-core.mdc (Objectifs – objectif de temps et édition).
- **Archivage :** `docs/design-request-goals/DESIGN_GOAL_TARGET_TIME.md`, `SPEC_GOAL_TARGET_TIME.md`, `MOCKUP_GOAL_TARGET_TIME_*.html` → `docs/archive/design-goal-target-time/`. Référence courante : **Project_context.md §4.7**, **docs/DESIGN_SYSTEM.md**, **docs/I18N.md**.
...
✅ **8 mai 2026 – Refonte modale entraînement (Objectif/Réalisé hero, feedback coloré) – Mode Analyste :**
- **Livraison :** Refonte visuelle complète de la modale entraînement (athlète et coach lecture seule). (1) Au-dessus du formulaire athlète et dans toute la modale lecture coach : nouveau composant **`WorkoutTargetActualCards`** — deux cartes côte-à-côte « Objectif » (light) et « Réalisé » hero (dark forest) avec **chips delta** entre les deux (`±M:SS/km` pour l’allure, `±N km/h` pour la vitesse, `±N,N km` pour la distance, `±N min` ou `±NhMM` pour le temps, `±N m` pour D+) et **commentaire athlète intégré** au bas de la card Réalisé (séparateur si métriques au-dessus). Live preview athlète : la card Réalisé se met à jour en temps réel pendant la saisie. (2) Tuiles feedback coach lecture seule via **`WorkoutFeedbackSummary`** : Ressenti / Intensité (RPE) / Plaisir distribués full-width (`flex-1`) avec **couleurs sémantiques par valeur** (rouge → vert pour 1–5, vert → orange → rouge pour 1–10). (3) Picker feedback athlète (`WorkoutFeedbackSection`) : même dégradé appliqué au bouton sélectionné (icône + label colorés pour Ressenti/Plaisir, fond plein pour Intensité). (4) Polissage : badge sport en-tête réduit ~15 % (text-xs / w-3 h-3), alignement vertical des rangées (`min-h-7`), valeurs d’allure réalisée en blanc, chip delta centrée (`leading-none`).
- **Fichiers :** `components/workout-modal/WorkoutTargetActualCards.tsx` (nouveau), `components/workout-modal/WorkoutFeedbackSummary.tsx` (nouveau), `components/workout-modal/WorkoutFeedbackSection.tsx`, `components/workout-modal/views/CoachReadOnlyWorkoutModalView.tsx`, `components/workout-modal/views/AthleteWorkoutModalView.tsx`, `components/WorkoutModal.tsx`, `lib/workoutFormatting.ts` (nouveau, tests Vitest `lib/workoutFormatting.test.ts`), `lib/workoutFeedbackColors.ts` (nouveau, source unique des dégradés feedback), `messages/fr.json`, `messages/en.json` (sous-namespace `workouts.summary.*`).
- **Doc :** `Project_context.md` §4.5 (Athlete can / Coach can / Feedback colors), `docs/DESIGN_SYSTEM.md` (WorkoutTargetActualCards, WorkoutFeedbackSummary, WorkoutFeedbackSection, Feedback colors `lib/workoutFeedbackColors.ts`, formats `lib/workoutFormatting.ts`), `docs/I18N.md` (sous-namespace `workouts.summary.*`).
- **Archivage :** `docs/design-workout-modal-revamp/` → `docs/archive/design-workout-modal-revamp/` (4 mockups HTML US1–US4). Référence courante : **Project_context.md §4.5**, **docs/DESIGN_SYSTEM.md**, **docs/I18N.md**.
...
| **Statut séance, modales entraînement (en-tête création/édition/lecture seule, tuile sport réduite ~15 %, date à gauche), cartes « Objectif vs Réalisé » + chips delta + commentaire intégré (`WorkoutTargetActualCards`), total « fait », retour athlète (ressenti, intensité, plaisir) coloré (tuiles `WorkoutFeedbackSummary` + picker `WorkoutFeedbackSection`, dégradés `lib/workoutFeedbackColors.ts`, formats `lib/workoutFormatting.ts`)** | **`Project_context.md` §4.5** (Workouts, Feedback colors), **`docs/DESIGN_SYSTEM.md`** (Modal, WorkoutModal, WorkoutTargetActualCards, WorkoutFeedbackSummary, WorkoutFeedbackSection, Feedback colors) |
