...
3. [Composants](#composants)
   ...
   - [CoachAthletesOnboardingPanel](#coachathletesonboardingpanel)
   - [CoachOnboardingStepVisual](#coachonboardingstepvisual)
   - [CoachOnboardingGhostAthleteTiles](#coachonboardingghostathletetiles)
  ...
  - [Drawer](#drawer)
   ...
   - [AthleteStatsAnnualSummary](#athletestatsannualsummary)
4. [Icônes](#icônes)
...
Deux cartes responsive côte-à-côte (`grid-cols-1 md:grid-cols-2`) qui résument l'**Objectif** (planifié) et le **Réalisé** (saisi par l'athlète) d'une séance. Utilisé dans `AthleteWorkoutModalView` (avec live preview), `CoachReadOnlyWorkoutModalView` (lecture seule) et **`AthleteLoggedCoachReadOnlyView`** (activité saisie par l'athlète : prop **`hideTargetCard`** — carte Objectif masquée, **pas de chips delta** de comparaison).
...
| `hideTargetCard` | `boolean` (optionnel, défaut `false`) | Masque la carte Objectif ; utilisé pour les activités `planned_by = 'athlete'` (pas d’objectif coach, pas de delta). |
...
**Fichier :** `components/AuthSubmitButton.tsx`

Bouton submit pour formulaires auth (`login`, `signup`) avec état loading via `useFormStatus().pending`. À placer **en enfant direct** du `<form action={serverAction}>`.
...
- Props : `children` (libellé repos), `loadingText` (libellé pendant soumission), `variant`, `fullWidth`, `disabled`, `className`
- i18n loading : `auth.loggingIn`, `auth.signingUp` (aligné sur `auth.loggingOut`, `auth.oauthRedirecting`)
- Surfaces : page `/login`, modale `LoginForm`, `EmailValidatedModal`
- Champs du formulaire restent éditables pendant le loading (seul le bouton change d'état)

#### SocialAuthButtons

**Fichier :** `components/SocialAuthButtons.tsx`

Boutons de connexion / inscription sociale (**Google uniquement** ; pas de bouton Apple en v1). Utilisé en tête des modales `LoginForm` et de la page `/login`, au-dessus du séparateur `AuthDivider` (deux traits + « ou ») puis du formulaire email.
...
- `intent` : `'login' | 'signup'` (passé à la server action `startGoogleOAuth`)
- i18n : `auth.continueWithGoogle`, `auth.oauthRedirecting`
- Pages post-OAuth : `/[locale]/auth/complete-signup`, `/[locale]/auth/link-account`
- Composants associés : `OAuthCompleteSignupForm` (rôle + CGU), `OAuthLinkAccountForm` (liaison compte existant), `AuthRolePicker`, `AuthLegalConsent`
- Callback OAuth racine (hors i18n) : `/auth/callback` — voir `Project_context.md` §4.1 et `DEPLOYMENT_NOTES.md` § Connexion Google

#### Modales auth dérivées

- **EmailValidatedModal** (`components/EmailValidatedModal.tsx`) : modale affichée après confirmation d’email (landing avec `?emailConfirmed=1`). Taille `md`, titre i18n « Email validé », message « Vous pouvez vous connecter », formulaire de connexion (email, mot de passe, `AuthSubmitButton` avec `auth.loggingIn`) dans la modale. Utilise `Modal`, `Input`, `AuthSubmitButton`, action `login` ; fermeture par overlay/Escape. i18n : `auth.emailValidatedTitle`, `auth.emailValidatedMessage`.
...
**Fichier :** `components/CoachAthletesOnboardingPanel.tsx`

Panneau **onboarding coach** sur `/dashboard/athletes` : checklist 3 étapes (profil, offre, premières demandes) avec **un CTA par étape incomplète**, mini-visuels (`CoachOnboardingStepVisual`) et zone aperçu (`CoachOnboardingGhostAthleteTiles`). Variante **prête** quand profil complet + offre `published`. Affiché seulement si **0 athlète actif**, **0 demande en attente**, pas d’erreur de chargement. i18n : `athletes.onboarding`.

#### Props

| Prop | Type | Description |
|------|------|-------------|
| `locale` | `string` | Locale active |
| `isProfileComplete` | `boolean` | `isCoachProfileComplete(profile)` |
| `hasPublishedOffer` | `boolean` | Au moins une offre `published` |

---

### CoachOnboardingStepVisual

**Fichier :** `components/CoachOnboardingStepVisual.tsx`

Mini-visuel décoratif par étape : `profile` (fiche), `offer` (carte offre), `requests` (tuile demande + micro-fantômes athlètes). États : `current`, `done` (compact), `upcoming` (atténué). `aria-hidden`, non interactif. Aligné sur **`docs/archive/design-coach-athletes-onboarding/MOCKUP_US_COACH_ONB_01_CHECKLIST.html`**.

#### Props

| Prop | Type | Description |
|------|------|-------------|
| `variant` | `'profile' \| 'offer' \| 'requests'` | Type de visuel |
| `state` | `'current' \| 'done' \| 'upcoming'` | Style |
| `offerPriceSampleLabel` | `string` (optionnel) | Exemple prix décoratif (`athletes.onboarding.stepVisual.offerPriceSample`) |
| `requestPlaceholderLabel` | `string` (optionnel) | Sous-texte visuel demande (étape 3 active) |
| `className` | `string` (optionnel) | Classes conteneur |

---

### CoachOnboardingGhostAthleteTiles

**Fichier :** `components/CoachOnboardingGhostAthleteTiles.tsx`

Grille de **1 à 3** tuiles fantômes alignées sur `AthleteTile` (skeleton, bordure dashed, opacité réduite). Preview non cliquable sous la checklist.
...
Menu compte **desktop (md+)** pour l’**athlète** : trigger avatar + nom tronqué + chevron ; panneau aligné sous le trigger (`absolute right-0`, `z-40`), fermeture clic extérieur / Escape / navigation. Contenu : **appareils connectés** (si `NEXT_PUBLIC_ENABLE_ATHLETE_STRAVA_DEVICES=true`, voir `lib/featureFlags.ts`), Mon coach (si `coach_id`), historique souscriptions ; **séparateur** ; **Contactez-nous** (`/contact`, `getContactPublicNavItem`) ; **séparateur** ; Mes informations (`/dashboard/profile`) ; **LogoutButton**. Styles de lignes alignés sur `DashboardNavLinks` (actif = vert forêt). Source des entrées : `getAthleteAccountNavItems`, `getAthleteProfileNavItem` dans `lib/dashboardNavConfig.ts`.
...
**Fichiers :** `components/landing/*`, `lib/landingConfig.ts`, `lib/landingScreenshots.ts`, `lib/landingTabStyles.ts`, `app/[locale]/page.tsx`

Sections composées pour visiteurs non connectés (Option B validée). Captures produit dans `public/landing/{fr|en}/*.webp` ; helper `getLandingScreenshotSrc(locale, id)`.

| Composant | Rôle |
|-----------|------|
| **`LandingHero`** | Hero split : copy + `AuthButtons` hero + `LandingPricingLink` + `LandingHeroVisual` |
| **`LandingHeroVisual`** | Stack desktop (calendrier athlète devant, coach derrière) ; 1 capture mobile |
| **`LandingScreenshotFrame`** | Cadre image réutilisable (`next/image`, bordure, ombre) |
| **`LandingShowcaseTabs`** | Client — 6 onglets scrollables (3 athlète / 3 coach), panneau texte + capture |
| **`LandingAudienceCards`** | Cartes « Je suis athlète / coach » + mini-captures + CTAs |
| **`LandingHowItWorks`** | 3 étapes numérotées |
| **`LandingFinalCta`** | Bandeau vert + `AuthButtons variant="ctaBand"` + lien tarifs |
| **`LandingPricingLink`** / **`LandingPricingLinkClient`** | Lien secondaire vers `/pricing` (server vs client) |
| **`LandingSignupButton`** | Client — CTA libellé custom → `LoginModal` signup |

**`AuthButtons`** : variantes `default`, `hero`, `drawer`, **`ctaBand`** (CTA final sur fond vert).

**i18n :** namespace `landing` — `hero`, `pricingLink`, `showcase`, `audience`, `howItWorks`, `cta`, `footer`.

**Mockups :** `docs/archive/design-landing-revamp/MOCKUP_US_LANDING_*.html`

---

### Pages FAQ publiques (`/faq/athlete`, `/faq/coach`)

**Fichiers :** `components/public/PublicPageShell.tsx`, `components/public/PublicMarketingFooter.tsx`, `app/[locale]/faq/*/page.tsx`, `app/[locale]/privacy/page.tsx`, `app/[locale]/terms/page.tsx`, `app/[locale]/contact/page.tsx`, `lib/faqPublicConfig.ts`
...
Graphique **volume réalisé** (Nivo `ResponsiveLine`, import client ; montage après première frame pour mesure). **Courbe A1 (#119)** : spline `monotoneX`, quadrillage horizontal seul, dégradé area sous l’**année la plus récente** (`lib/athleteStatsNivoLayers.tsx`), lignes stylées (3e année pointillée), points au survol slice, tooltip pill (`AthleteStatsSliceTooltip`). **Légende inline** en en-tête (`AthleteStatsChartHeader`). Plusieurs années = plusieurs lignes (`ATHLETE_STATS_LINE_COLORS`). **Pas de légende Nivo** sous le graphe. **Tranches verticales** (`enableSlices="x"`) : survol = `sliceTooltip`. **Encart A2** : `AthleteStatsAnnualSummary` (barres fines inline, total à droite). Helpers : `lib/athleteStatsChartUi.ts`.
...
**Skeletons :** `components/athlete/AthleteStatsChartSkeleton.tsx` — `AthleteStatsChartPlotSkeleton` (zone courbe gradient + courbes factices) ; `AthleteStatsChartFullSkeleton` (+ encart annuel A2 factice) lors du refetch.

**Thème Nivo :** `lib/athleteStatsNivoTheme.ts` + variables `--chart-*` dans `app/globals.css` (quadrillage / axes / tooltip / area), sans hex dans le composant. **Layers custom :** fonctions plain dans `lib/athleteStatsNivoLayers.tsx` (pas `React.memo()` — Nivo exige `typeof layer === 'function'`).

---

### AthleteStatsAnnualSummary

**Fichier :** `components/athlete/AthleteStatsAnnualSummary.tsx`

Encart **volume annuel** sous la courbe stats (variante A2, #119) : pastille couleur, année, barre de progression fine (`h-1.5`, % du max des totaux affichés), total formaté à droite. Fond `bg-white`, bordure `stone-200`. Données via `buildAnnualVolumeRows` (`lib/athleteStatsChartUi.ts`).
...
- **Top bar dashboard** : barre en haut (`DashboardTopBar`) — logo My Sport Ally à gauche, liens de navigation au centre (tablette/desktop, centrés). **Athlète** : liens principaux seuls au centre ; à droite `AthleteAccountMenu` (menu compte : appareils connectés si feature flag Strava activé, coach si lié, historique ; séparateur ; Contact ; Mes informations ; déconnexion). **Coach** : à droite `CoachAccountMenu` (Mes informations, Contact, déconnexion). **Admin** : lien profil direct à droite. **Mobile :** titre de la page au centre, bouton hamburger à droite ; **Drawer** athlète = liens secondaires (dont appareils si flag), contact, Mes informations, déconnexion ; **coach** = liste nav, carte profil, contact, déconnexion ; **admin** = liste nav, carte profil, déconnexion. **Admin** : nav = Gestion des membres + Design System uniquement. Fichiers : `DashboardTopBar.tsx`, `AthleteAccountMenu.tsx`, `CoachAccountMenu.tsx`, `DashboardNavLinks.tsx`, `Drawer.tsx`, `lib/dashboardNavConfig.ts`, `lib/featureFlags.ts`. **Pages dashboard** : `DashboardPageShell` fournit uniquement le padding de contenu — pas de titre en tête de page ni conteneur carte. Fichier : `components/DashboardPageShell.tsx`.
...
- **Page « Mes athlètes » (My Athletes)** (`/dashboard/athletes`, coach) : page dédiée avec son propre skeleton (titre, filtre, grille). Titre de page : « My Athletes » / « Mes Athlètes » (namespace `navigation.athletes`). **Onboarding coach (empty state) :** si **0 athlète actif**, **0 demande en attente** et liste chargée OK → **`CoachAthletesOnboardingPanel`** (checklist profil / offre / demandes, CTA par étape incomplète, visuels par étape, fantômes athlètes) ; **masqué** dès qu’une demande est en attente ou qu’un athlète est actif. Puis section **Demandes en attente** (si présente), puis liste. Filtre par nom (champ de recherche inline à côté du titre « Mes athlètes (X) », temps réel, insensible à la casse et aux accents ; si aucune correspondance : icône loupe, `athletes.searchNoMatchWithQuery`, bouton **Effacer la recherche** `athletes.searchClear`). Effectifs à côté des titres de section (« Mes athlètes (X) », « Demandes en attente (X) »). **Erreur chargement liste athlètes :** message dans un encart `bg-palette-danger-light`, bordure `palette-danger`, texte `palette-danger-dark` (`athletes.listLoadError`), pas le panneau onboarding. **Demandes en attente (PendingRequestTile)** : bordure gauche ambre ; en-tête : avatar, **nom · offre** (offre en police plus petite), badges sport, boutons Discuter / Refuser / Accepter ; corps en **2 colonnes** : « Message de l’athlète » (coaching_need) | « Objectifs et volume (athlète) » (temps à allouer + volumes par sport depuis le profil : lignes « Sport : valeur unité » avec bordure gauche et icône par sport, D+ dans le même bloc que Course ; si vide « Non renseigné »). Modales de confirmation avant Refuser/Accepter ; sur mobile les boutons sont en bas de la tuile. Grille des tuiles athlètes : 1 colonne par défaut, 2 colonnes à partir de `sm`, **3 colonnes à partir de `xl` uniquement**. Fichiers : `app/[locale]/dashboard/athletes/page.tsx`, `CoachAthletesOnboardingPanel.tsx`, `CoachAthletesListWithFilter.tsx`, `PendingRequestTile.tsx`, `lib/coachProfileCompletion.ts`.
...
- **Stats athlète (Nivo)** : `lib/athleteStatsNivoTheme.ts`, `lib/athleteStatsColors.ts`, `lib/athleteStatsChartUi.ts`, `lib/athleteStatsNivoLayers.tsx`, `components/athlete/AthleteStatsVolumeChart.tsx`, `components/athlete/AthleteStatsAnnualSummary.tsx`, `components/athlete/AthleteStatsChartHeader.tsx`, `components/athlete/AthleteStatsSliceTooltip.tsx`
