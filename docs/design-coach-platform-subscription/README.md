...
→ Livraisons **Développeur** (mai 2026) : modale offres + **évolution** carte abonnement (montant, **prochain paiement** avec repli date Stripe, essai gratuit, fin tolérance **074**) + **adresse de facturation** (**US-COACH-MYSA-BILL-01**) documentées dans **README** ci-dessus, **`Project_context.md`**, **`docs/DESIGN_SYSTEM.md`**, **`docs/I18N.md`**. Maquette grâce 3 j **archivée** sous `docs/archive/design-coach-platform-subscription-grace/`. Évolutions ultérieures (nouveaux `price_id`, offre « encaissements ») : ajouter les clés sous **`byPriceId`** et les IDs dans l’env, puis mise à jour **`Project_context.md`** / **`docs/I18N.md`** si besoin.
