...
**Statut :** Phase Designer **2** + livraisons **Développeur** mai–juin 2026 (catalogue, essai, facturation, **résiliation + tuile moderne**, **prérequis Checkout** — **US-MYSA-CHECKOUT-PREQ-01…03** **livré**). Proposition **A** (modale catalogue avant Stripe), essai lancement et **US-MYSA-CANCEL-01…05** / **US-MYSA-SUB-UI-02** **implémentées**. **`coachMsaOffers.byPriceId`** (FR + EN). **Page abo :** tuile gérée **`active`/`trialing`** (+ fin programmée) ; bannière impayé **`past_due`/`unpaid`** ; grille offres si **`shouldShowCoachPlatformOfferGrid`** (**`offersTitle`** `sr-only`).  
...
## User stories → mockups (prérequis Checkout — juin 2026, Phase 2 validée PO)

| US | Fichier | Rôle |
|----|---------|------|
| **US-MYSA-CHECKOUT-PREQ-01** | `MOCKUP_US_MYSA_CHECKOUT_PREREQ_01_MODAL_SWAP.html` | **Modale catalogue** : clic **Souscrire** → si prérequis manquants, **bascule contenu** (même `Modal` `3xl`) ; **Retour aux offres** ; redirect Stripe après save ; nominal = redirect direct |
| **US-MYSA-CHECKOUT-PREQ-02** | `MOCKUP_US_MYSA_CHECKOUT_PREREQ_02_SUB_PAGE_MODAL.html` | **Page abonnement** : clic **Souscrire** → **modale dédiée** (`Modal` `md`) par-dessus la page ; **Annuler** ferme la modale |
| **US-MYSA-CHECKOUT-PREQ-03** | `MOCKUP_US_MYSA_CHECKOUT_PREREQ_03_FORM.html` | **Formulaire commun** : prénom + nom + adresse facturation (tous champs visibles, pré-remplissage) ; validation, pending, erreurs ; save profil + Stripe puis checkout |

**Arbitrages PO (juin 2026) :** prénom **et** nom obligatoires · **tous les champs** affichés · ligne adresse 2 optionnelle · gate **systématique** avant redirect Stripe (modale + page) · modale offres = bascule contenu · page abo = modale overlay.
...
- Modale choix d’offre avant Checkout : **`components/CoachPlatformSubscribeOffersModal.tsx`** (chargement **`loadCoachPlatformCatalogForCoach`** dans **`app/[locale]/dashboard/athletes/coachPlatformActions.ts`**), intégrations **`PendingRequestTile`**, **`CoachAthletesBillingOverlay`**, **`CoachAthleteBillingBlocked`**. **Prérequis Checkout (US-MYSA-CHECKOUT-PREQ-01…03)** — **livré** : gate nom + adresse avant redirect ; bascule contenu modale offres ; **`CoachPlatformCheckoutPrerequisitesModal`** sur page abo ; **`CoachPlatformCheckoutPrerequisitesForm`**, **`CoachPlatformBillingAddressFields`** ; actions **`coachPlatformCheckoutPrerequisitesActions.ts`** ; **`lib/coachPlatformCheckoutPrerequisites.ts`** ; **`persistCoachPlatformStripeBillingForCoach`** ; garde **`evaluateCoachPlatformCheckoutPrerequisitesForCoach`** dans **`createCoachPlatformCheckoutSession`**.
...
→ **Prérequis Checkout** (**US-MYSA-CHECKOUT-PREQ-01…03**) — **livré** (juin 2026) : voir **Implémentation** ci-dessous.  
