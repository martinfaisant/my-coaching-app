...
- **Coach ↔ platform (Stripe, migration `073_coach_platform_subscription.sql`):** the coach pays **My Sport Ally** via **Stripe Checkout** (subscription). Table `coach_platform_subscriptions`, webhook **`POST /api/webhooks/stripe`**, RPC **`coach_platform_access_granted`** (access rules including grace for `past_due` / `unpaid`). Without access: **Mes athlètes** and **athlete calendars** blocked, **chat** restricted, **Accepter** on pending requests opens a paywall modal then Checkout. **Dedicated page** **`/dashboard/coach-platform-subscription`** (`app/[locale]/dashboard/coach-platform-subscription/page.tsx`, `DashboardPageShell`): subscription status, optional **multi-offer** grid from **`STRIPE_COACH_PLATFORM_PRICE_IDS`** (Stripe Prices API via `lib/stripeCoachPlatformCatalog.ts`), **billing history** (invoices / failed charges / refunds via `lib/stripeCoachPlatformBillingHistory.ts`), grace alert when `past_due` / `unpaid` and access still granted by RPC. **`createCoachPlatformCheckoutSession`** (`app/[locale]/dashboard/athletes/coachPlatformActions.ts`) whitelists `priceId` and **`returnPath`** (`lib/coachPlatformCheckoutReturnPath.ts`, test `lib/coachPlatformCheckoutReturnPath.test.ts`) so the coach returns to the **same dashboard path** after Checkout when allowed; base URL for success/cancel still from **`lib/checkoutReturnOrigin.ts`** (allowed host / preview / env fallback). Post-return UI in coach dashboard layout: **`CoachPlatformStripeBanner`**, **`CoachPlatformCheckoutVerification`** (client + `verifyCoachPlatformCheckoutSession`). Client offer tiles: **`CoachPlatformSubscriptionOffers`**. i18n: **`coachMsaSubscription`** (subscription page + nav label **`navigation.coachPlatformSubscription`**), **`coachPlatform`** / **`coachPlatform.verify`** (paywall, banners, checkout validation).
