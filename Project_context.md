...
- **Connexion / inscription Google (OAuth)** : bouton **Continuer avec Google** sur la page et la modale `/login` (`SocialAuthButtons` → `startGoogleOAuth`). Flux Supabase `signInWithOAuth` ; callback racine **`/auth/callback`** (`app/auth/callback/route.ts`, hors préfixe i18n — seul ce chemin est exclu du middleware i18n, pas `/auth/complete-signup`). **Nouveau compte Google** sans ligne `profiles` : redirection **`/[locale]/auth/complete-signup`** — choix du rôle (athlète / coach, **aucun rôle présélectionné** à l’inscription), case CGU obligatoire (`AuthRolePicker`, `AuthLegalConsent`, `OAuthCompleteSignupForm`) puis `completeOAuthSignup` (insert profil). État intermédiaire : `user_metadata.oauth_signup_pending` ; pas de création auto de profil tant que l’inscription OAuth n’est pas finalisée (`utils/auth.ts`). **Prénom / nom** : extraits des métadonnées Google (`full_name` / `name`, repli `given_name` / `family_name` via `lib/googleUserMetadata.ts`) à la finalisation ; **pas d’import de photo Google** (avatar reste saisissable dans Mon profil). **Retour Google avec profil existant** : dashboard + backfill des noms vides (`lib/authOAuth.ts`). **Collision email** (erreur OAuth « compte existant ») : redirection **`/[locale]/auth/link-account`** — saisie mot de passe puis `linkGoogleAccount` (`signInWithPassword` + `linkIdentity`). **Apple** : hors périmètre v1 (pas de bouton). i18n : namespace **`auth`** (`continueWithGoogle`, `finalizeAccountTitle`, `linkGoogleAccountTitle`, `auth.errors.oauth*`). Déploiement Google Cloud + Supabase : `DEPLOYMENT_NOTES.md` § Connexion Google. Maquettes design archivées : `docs/archive/design-auth-social-login/`.
