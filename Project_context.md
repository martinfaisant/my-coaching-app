...
- **Objectifs séance (temps / distance / allure) :** Les **unités par sport** du profil coach (`profiles.workout_primary_metric_by_sport`) fixent la métrique **obligatoire** à la saisie (temps ou distance) pour les sports concernés. Côté client, `components/workout-modal/useWorkoutFormReducer.ts` enchaîne les calculs entre durée, distance et allure/vitesse (selon `workoutHasPaceField` dans `lib/sportsRegistry.ts`) en s’appuyant sur le **dernier champ modifié**, pour permettre l’édition sans réécrasement intempestif et le **vidage** volontaire des champs. Côté serveur, `validateWorkoutFormData` (`lib/workoutValidation.ts`) renvoie des codes d’erreur traduits via **`translateWorkoutFormValidationError`** et le namespace **`workouts.validation`** dans `createWorkout` / `updateWorkout` (locale via `getLocale()`). Cadrage UX : `docs/design-coach-workout-units/`.
