...
- **Stripe:** **Coach platform** subscription implemented (Checkout + webhooks + migration 073; Customer **`preferred_locales`** and Checkout session **`locale`** follow app **`[locale]`** via **`ensureCoachPlatformStripeCustomerForCheckout`**). Athlete–coach **`subscriptions`** rows remain non-Stripe (snapshot / structural only).
