...
- Page marketing / legale publique indexable : ajouter le chemin dans `SEO_PUBLIC_PATHS` (`lib/seoPublicRoutes.ts`) et `generateMetadata` avec **`buildPublicPageMetadata`** (`lib/seoMetadata.ts`) ; clés **`metadata.*`** FR/EN ; JSON-LD FAQ via **`buildFaqPageJsonLd`** (`lib/faqPublicConfig.ts`) sur FAQ et `/pricing` ; home : **`buildHomeJsonLdGraph`** (`lib/seoJsonLd.ts`) + profils sociaux **`lib/socialLinks.ts`** (`sameAs`, footer, contact, `llms.txt`) ; voir `DEPLOYMENT_NOTES.md` § Referencement et **Project_context.md** §4.14–§4.15.
