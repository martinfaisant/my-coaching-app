...
Les **visiteurs** voient une **page d'accueil marketing** (`/` et `/en`) avec captures produit (calendrier, stats, recherche coach, espace coach), onglets de découverte et invitation à créer un compte — détail **Project_context.md** §4.13. Ils peuvent **parcourir l'annuaire coach public** sur **`/coaches`** (filtres sport / langue / nom, fiches **`/coaches/[id]`** indexables) et envoyer une demande après création de compte athlète — §4.16. Ils peuvent consulter les **tarifs coach plateforme** sur **`/pricing`** (abonnement MySportAlly requis seulement à l’acceptation d’un athlète — §4.12) et les **FAQ publiques** **`/faq/athlete`** et **`/faq/coach`** (§4.15). Ils peuvent aussi contacter le support via **Contact** (`/contact`), avec accusé de réception (référence MSA) et notification e-mail côté équipe (voir variables Resend ci-dessous). Les utilisateurs **connectés** qui ouvrent l’accueil marketing **`/`** ou **`/en`** sont **redirigés** vers la page d’entrée du tableau de bord selon leur rôle (détail : **Project_context.md** §4.0, code **`lib/dashboardEntryPath.ts`**).
